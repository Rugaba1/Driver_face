...
// Initializing program component
...
static esp_err_t start_car_handler(httpd_req_t *req) {
   lcd.clear();
   lcd.setCursor(0,0);
  ...
  httpd_resp_send(req, "Car started", HTTPD_RESP_USE_STRLEN);
  
  return ESP_OK;
}

static esp_err_t stop_car_handler(httpd_req_t *req) {
   lcd.clear();
   lcd.setCursor(0,0);
  ...
  httpd_resp_set_type(req, "text/plain");
  httpd_resp_send(req, "Car stopped", HTTPD_RESP_USE_STRLEN);
...
static esp_err_t gps_handler(httpd_req_t *req) {
  if(gpsSerial.available()>0){
    ...
    if(gps.location.isUpdated()){
      httpd_resp_set_type(req, "text/plain");
      String response = "{lat:" + String(gps.location.lat(), 6) + ",log:" + String(gps.location.lng(), 6) + "}";
    httpd_resp_send(req, response.c_str(), HTTPD_RESP_USE_STRLEN);
      }
     else{
      httpd_resp_set_type(req, "text/plain");
      httpd_resp_send(req, "GPS data not updated", HTTPD_RESP_USE_STRLEN);
      }
    }
    else{
      httpd_resp_set_type(req, "text/plain");
      httpd_resp_send(req, "No GPS data found", HTTPD_RESP_USE_STRLEN);
      }
  return ESP_OK;
}
